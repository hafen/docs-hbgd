...
This package provides a few modeling techniques and tries to find good defaults when applied across many subjects.  The supported techniques include functional smoothing using `smooth.basisPar()` in the `fda` package and `face.sparse()` in the `face` package, robust loess smoothing via `loess()`, splines via `gam()`, robust linear models via `rlm()`, and the ability to plug in new modeling techniques is available.

The goal of the package is to provide a set of best methods that can be used
...
It could well be that different data sets will always warrant different choices.  To assist with making these choices, this package also provides visualization methods to help assess the goodness-of-fit of each approach.  See the [trelliscope](#trelliscope) section for more details.
...
fit <- fit_trajectory(subset(cpp, subjid == 2), y_var = "htcm", method = "fda")
```
...
# manual cache
cppt <- fit_all_trajectories(cpp, y_var = "htcm", z_bins = -2, method = "rlm")
...
plot(cppt[["subjid=24"]]$value, hover = c("agedays", "htcm"))
```
...
We see this is a "distributed data frame" (DDF) - a special kind of DDO where each subset is a data frame.  It has 5 subsets.  We can look at the keys for these subsets with:

```{r fit9, eval=TRUE, echo=TRUE}
getKeys(cppcp)
```

Thus we see subset 1 is "not stunted at 1 year, not stunted at 2 years", etc.  Now we can investigate the data for the subjects in each subset for further exploration, analyses, or visualization.
